...
function useHeroState() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setState((current) => (current + 1) % heroStates.length), 5200);
...
function HeroVisual({ state }: { state: number }) {
  const notification = ["New request near you", "Reward earned", "USSD access available", "Donor match found", "FHIR connection ready", "Platform connected"][state];
  const match = ["Yusif, you may be able to help", "Your donation made an impact", "Access Hemova anywhere", "Yusif, you may be able to help", "Your data stays protected", "One connected journey"][state];
  return <div className="hero-visual-state" key={`hero-visual-${state}`}><div className="hero-halo" /><div className="notification-card"><BellRing size={17} /><span>{notification}</span><span className="notification-time">now</span></div><div className="hero-state-card">{state === 0 ? <RequestCard /> : state === 1 ? <><span className="hero-state-icon gold"><Gift /></span><p className="hero-state-label">Hemova recognition</p><strong>+200 points</strong><small>Donation certificate unlocked</small><div className="hero-state-progress"><span /></div><b>Milestone 3 of 5</b></> : state === 2 ? <><span className="hero-state-icon green"><Smartphone /></span><p className="hero-state-label">USSD access</p><strong>*800#</strong><small>Discover opportunities without a smartphone</small><div className="hero-state-code"><span>1</span> Find a donation centre<br /><span>2</span> Respond to a request</div></> : state === 3 ? <><span className="hero-state-icon purple"><Network /></span><p className="hero-state-label">Smart matching</p><strong>Suitable donor found</strong><small>O+ · 4.8 km away · Available today</small><div className="hero-match-meter"><span /><span /><span /><span /></div><b>Verified match</b></> : state === 4 ? <><span className="hero-state-icon blue"><Database /></span><p className="hero-state-label">Healthcare integration</p><strong>FHIR R4 ready</strong><small>Connected systems exchange only authorized data.</small><div className="hero-fhir-row"><span>Hospital</span><i /><span>Hemova</span><i /><span>Blood bank</span></div></> : <><span className="hero-state-icon red"><HeartHandshake /></span><p className="hero-state-label">One ecosystem</p><strong>Everyone connected</strong><small>Donors · hospitals · blood banks · drives</small><div className="hero-ecosystem"><span>Donors</span><span>Hospitals</span><span>Centres</span></div></>}</div><div className="match-card"><div className="match-avatar">YA</div><div><strong>{match}</strong><span>Verified, coordinated and community-led</span></div><Check size={18} /></div></div>;
}
...
export function HemovaHome() {
 ...
 return <main id="top">
  <PublicNavbar />
  <ScrollReveal as="section" className="hero container"><div className="hero-copy"><DynamicHeroCopy state={heroState} /><ScrollReveal as="div" className="hero-buttons" delay={260}><a className="button" href="/signup">Become a donor <ArrowRight size={18} /></a><a className="button button-secondary" href="/requests">Request blood</a></ScrollReveal><ScrollReveal as="p" className="quiet-note" delay={340}><ShieldCheck size={16} /> Verified facilities. Your information stays protected.</ScrollReveal></div><ScrollReveal as="div" className="hero-visual" delay={120}><HeroVisual state={heroState} /></ScrollReveal></ScrollReveal>
  ...
  <ScrollReveal as="section" className="section process" id="how-it-works"><div className="container"><div className="section-heading centered"><p className="eyebrow">How Hemova works</p><h2>A clearer path from <em>intention to impact.</em></h2><p>Simple, verified coordination for people who want to help and the teams making donation possible.</p></div><StaggerReveal className="steps">{steps.map((step,i) => <div className="step" key={step}><span className="step-number">0{i+1}</span><h3>{step}</h3><p>{["Share the details you are comfortable with.", "We’ll show relevant requests and blood drives.", "Your centre completes the clinical screening.", "See your history and when to plan ahead."][i]}</p></div>)}</StaggerReveal></div></ScrollReveal>
